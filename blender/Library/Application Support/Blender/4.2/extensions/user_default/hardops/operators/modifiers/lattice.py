...
"""
    ...
    def execute (self, context):
        ...
        self.selected = [ob for ob in context.selected_objects if ob.type in {'MESH', 'CURVE', 'FONT', 'SURFACE', 'GPENCIL', 'GREASEPENCIL'}]
    ...
    def lattice_modifiers(obj):
        if obj.type == 'GPENCIL':
            ...
            return [modifier for modifier in obj.modifiers if modifier.type == 'GREASE_PENCIL_LATTICE']
        else:
    ...
    def add_lattice_modifier(self, context, obj, lattice_object):
        if obj.type == 'GPENCIL':
            ...
            lattice_modifier = obj.modifiers.new(name="Lattice", type='GREASE_PENCIL_LATTICE')
        else:
            lattice_modifier = obj.modifiers.new(name="Lattice", type='LATTICE')

        lattice_modifier.object = lattice_object

        if obj.type in {'MESH', 'GPENCIL', 'GREASEPENCIL'} and obj.mode in {'EDIT', 'EDIT_GREASE_PENCIL'}:
    ...
    def get_vert_coords(self, obj, context, matrix = Matrix()):
        ...
        elif obj.type in {'GPENCIL', 'GREASEPENCIL'} and obj.mode in {'EDIT_GPENCIL', 'EDIT_GREASE_PENCIL'}:
            lattice_verts = obj.vertex_groups.new(name='HardOps_Lattice')
            group_idx = lattice_verts.index
