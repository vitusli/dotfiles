...
class TransformEdgeConstrained(bpy.types.Operator):
    ...
    def draw_HUD(self, args):
        ...
        if self.is_zero_scaling:
            ...
            height -= 20 * scale
            draw_label(context, title=f"{self.constrain_mode.title().replace('_', ' ')}", coords=self.mousepos + Vector((20, height)), alpha=0.5, center=False)

        if self.is_axis_locking:
            axis = self.transform_axis[-1]
            height -= 20 * scale
            draw_label(context, title=f'LOCAL {self.transform_axis}', coords=self.mousepos + Vector((20, height)), color=axis_color_mapping[axis], center=False)
