...
class GroupPosesUIList(bpy.types.UIList):
    bl_idname = "MACHIN3_UL_group_poses"
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        row.operator('machin3.retrieve_group_pose', text='', icon='ARMATURE_DATA', emboss=False).index = index
        row.alert = False

        r = row.row(align=True)

        if isactive:
            op = r.operator('machin3.sort_group_pose', text='', icon='TRIA_UP', emboss=False)
            op.direction = 'UP'
            op.index = index

            op = r.operator('machin3.sort_group_pose', text='', icon='TRIA_DOWN', emboss=False)
            op.direction = 'DOWN'
            op.index = index
        ...
        layout.operator('machin3.remove_group_pose', text='', icon='X', emboss=False).index = index
