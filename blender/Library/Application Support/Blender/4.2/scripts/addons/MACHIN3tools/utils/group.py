...
def get_group_polls(context):
    ...
    batchposable = bool([obj for obj in active_group.children_recursive if obj.type == 'EMPTY' and obj.M3.is_group_empty]) if active_group else False
...
def set_pose_axis_and_angle(empty, pose, inceptions=[]):
    if not inceptions:
        inceptions = [p for p in empty.M3.group_pose_COL if p.uuid == '00000000-0000-0000-0000-000000000000' and p != pose]
...
def process_group_poses(empty, debug=False):
    ...
    for e in group_empties:
        inceptions = [p for p in e.M3.group_pose_COL if p.uuid == '00000000-0000-0000-0000-000000000000']
